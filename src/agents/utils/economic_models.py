...
class MonteCarloPortfolioSimulator:
    ...
    def simulate_portfolio(
        ...
        if (len(asset_names) > 1 and self.correlation_matrix is not None and 
            hasattr(self, 'correlation_assets') and len(self.correlation_assets) > 0):
    ...
    def generate_csv_data(
        ...
        if initial_values is None:
            initial_values = {name: 100000 for name in scenarios.keys()}
    ...
    def validate_model(self, test_data: Dict[str, pd.DataFrame], validation_period_years: float = 1.0) -> Dict[str, float]:
        """
